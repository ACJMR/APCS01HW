...
TO DO LIST 
===================================================================
$ add the functionality of detecting and working around punctuation  (uppercase---done.....consonant sounds (ie "th") --- done)
...
public class Pig {
	...
	/* Experimental code using scanner intended for translating whole sentences 
	String out = "";
	String newW = "";
	Scanner str = newScanner(w);
	while (str.hasNext()) {
	    newW += str.next();
	    str = str.substring(str.next().length()); 
	} while (newW.hasNext()) {
	    if (beginsWithVowel(newW.next())) {
		out += newW.next() + "way";
		newW = newW.substring(newW.hasNext().length());
	    } else {
		for (int count = 0; count < newW.next().length(); count++) {
		    if (! isAVowel(newW.next().substring(0, 1))) {
			newW.next();
	*/

	char firstLet = w.charAt(0); //converst 1st letter to char for caps testing
	...
	if (beginsWithVowel(w)) { // if the first letter of the word is a vowel, then the rules simply say to add "way" to the end of the input and return it
	    product = (w + "way");
	} else { // if the first letter is not a vowel, then we return the substring starting at the index of the first vowel, and add the substring of the first few consonants to the end and add "ay" and return the translated phrase
	    product = (w.substring(w.indexOf(firstVowel(w))) + w.substring(0, (w.indexOf(firstVowel(w)))) + "ay" ); 
	}

	if (capital){ //if word was capitalized...
	    return product.substring(0,1).toUpperCase() + product.substring(1); //change the first letter of product to uppercase
	}
	return product;
    }
